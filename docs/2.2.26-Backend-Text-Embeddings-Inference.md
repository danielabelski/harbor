...
2. Create a workspace, open its document manager, upload a file and **Move to Workspace > Save and Embed**, or drag a file into the chat. One `/v1/embeddings` request carries every chunk of the file (a 3,000-word document is 50+ inputs); TEI logs a single `openai_embed` line and the document appears under the workspace
3. Chat in the workspace; the retrieved chunks are listed as citations under the reply
