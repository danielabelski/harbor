...
- **REST API**: every UI action is available under `/api`
...
# Image and tag (the nvidia cross-file switches the tag to latest-gpu)
...
# Comma-separated whisper model sizes preloaded on startup and offered in the UI
...
rm -rf services/whishper/data/db   # WARNING: destroys all transcriptions
harbor up whishper
```
