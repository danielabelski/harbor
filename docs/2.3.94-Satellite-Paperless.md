...
Log in with `admin` / `admin` (see `HARBOR_PAPERLESS_ADMIN_*` below to change). The stack is two containers: `paperless` (web server, consumer, celery worker) and `paperless-valkey` (task broker). The database is SQLite stored under the data volume.
...
# Public URL, used for CSRF and for links; change when exposed via a different host
...
Files are created as `HARBOR_USER_ID:HARBOR_GROUP_ID` (`USERMAP_UID`/`USERMAP_GID`), so the data directories stay owned by your host user.
...
- Changing `HARBOR_PAPERLESS_ADMIN_*` after the first start does not update the existing user; change the password in the UI instead.
