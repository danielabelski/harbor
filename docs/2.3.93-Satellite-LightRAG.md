...
- The Ollama cross-file uses `qwen3.5:4b` for the LLM and `nomic-embed-text` for embeddings by default. A `lightrag-ollama-init` sidecar pulls the LLM and derives two copies of it, `lightrag/qwen3.5:4b` and `lightrag/qwen3.5:4b-query`, before LightRAG starts (they share the base model's blobs on disk; see [Backends](#backends) for why there are two). `harbor ollama pull nomic-embed-text` yourself, or let Ollama's default model list do it.
...
Stop the stack with `harbor down lightrag ollama` (or a bare `harbor down`). `harbor down lightrag` alone fails with `no such service: lightrag-ollama-init`, because the sidecar lives in the Ollama cross-file and Harbor's `down` only loads the compose files of the services you name; this is the same limitation that affects every cross-file sidecar in Harbor (e.g. `open-design-ollama-proxy`).
...
**Ollama is the recommended LLM backend, and the only one Harbor's runtime check passes on.** `./services/lightrag/check-graph.sh --stack ollama` ingests a paragraph and asks five different hybrid-mode questions about it, requiring grounded prose for at least four; on the shipped defaults it passed 5 of 5 consecutive runs (22 of 25 answers grounded, 0 keyword or extraction JSON). The llama.cpp path is experimental: the same check on `harbor up lightrag llamacpp tei` passed 3 of 3 runs with 13 of 15 answers grounded and 2 extraction-JSON answers; details under [Troubleshooting](#graph-mode-answers-come-back-as-high_level_keywords--or-entities--json) together with what causes it.
...
# harbor up lightrag ollama: base model pulled and derived by lightrag-ollama-init
...
- `harbor up lightrag ollama` puts query answers on their own derived model (`lightrag/<model>-query`, see [Backends](#backends)), so an answer never follows a JSON-mode call on its runner. Result on shipped defaults: 5 of 5 consecutive `check-graph.sh --stack ollama` runs passed, 22 of 25 hybrid answers grounded, none of them JSON (the misses were wrong-entity or "no information" prose). If you set `QUERY_LLM_MODEL` back to the extraction model, expect 0 to 2 grounded answers out of 5.
- `harbor up lightrag llamacpp tei` cannot split roles the same way (the router serves one model per name), which is why it stays experimental. Ingest and `naive` mode work (`check-graph.sh --stack llamacpp --ingest-only`). Measured hybrid answers on the shipped defaults (`unsloth/Qwen3.5-4B-GGUF:Q4_K_M`, ROCm `llama-server`, serialized calls): 3 of 3 consecutive `check-graph.sh --stack llamacpp` runs passed with 13 of 15 answers grounded; the 2 misses were extraction JSON. Retry a query that comes back as JSON, or run a `naive` query first.
...
Entity extraction makes many LLM calls per document; with a small CPU-bound model a single page can take minutes. Check `harbor logs ollama` (or `llamacpp`) for activity. On the Ollama path, `harbor logs lightrag-ollama-init` shows whether the model pull and the derived-model creation succeeded; if `HARBOR_LIGHTRAG_OLLAMA_MODEL` names a model that does not exist in the Ollama library, the sidecar fails and `harbor up` stops before LightRAG starts. Ingest calls that time out with `httpx.ReadTimeout` after several minutes mean the model is thinking: check that `HARBOR_LIGHTRAG_OLLAMA_REASONING_EFFORT` is still `none`.
