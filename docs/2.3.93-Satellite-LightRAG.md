...
- **Ollama-compatible API**: `/api/chat` and `/api/tags` expose the knowledge base as a model named `lightrag:latest`
...
- The Ollama cross-file uses `qwen2.5:0.5b` and `nomic-embed-text` by default; `harbor ollama pull` both before the first ingest, or point `HARBOR_LIGHTRAG_OLLAMA_MODEL` at a model you already have.
...
Small models (under ~3B) frequently fail LightRAG's structured entity extraction. Graph-based modes (`local`, `global`, `hybrid`) then answer `[no-context]` while `naive` mode still works. Use a 7B+ model for good graph quality.
...
The LLM extracted no entities from the documents; small models do this often. Retry with `naive` mode, or switch to a larger model (`harbor config set lightrag.ollama.model <model>`) and re-ingest.
