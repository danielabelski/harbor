...
| `harbor up lightrag llamacpp tei` (experimental) | llama.cpp, `HARBOR_LIGHTRAG_LLAMACPP_MODEL` | TEI |
| `harbor up lightrag llamacpp ollama` (experimental) | llama.cpp | Ollama |
...
**Ollama is the recommended LLM backend.** The llama.cpp cross-files are experimental: ingest and `naive` queries work, but graph-mode queries (`local`/`global`/`hybrid`/`mix`) intermittently come back as keyword JSON instead of an answer on ROCm builds of `llama-server`. See [Troubleshooting](#graph-mode-answers-come-back-as-high_level_keywords--on-llamacpp) for what is known.

The llama.cpp cross-files disable Qwen3-style thinking (`HARBOR_LIGHTRAG_LLAMACPP_THINKING=false`); with thinking on, a 4B model spends thousands of reasoning tokens per extraction call and regularly runs into LightRAG's per-call timeout. They also run one LLM call at a time (`HARBOR_LIGHTRAG_LLAMACPP_MAX_ASYNC=1`, `MAX_PARALLEL_INSERT=1`) with a 180 s per-call timeout and a 4096-token completion cap: `llama-server` serves a single model with a few slots, and LightRAG's default fan-out of 4 async calls x 2 parallel inserts otherwise queues extraction workers into its 480 s worker timeout, turning a one-paragraph ingest into an 8-minute wait. With these limits the same paragraph ingests in about 30 s.
...
Known limitation of the experimental llama.cpp path. LightRAG issues a keyword-extraction call (JSON mode) and then an answer call per graph-mode query; on ROCm builds of `llama-server` (Harbor's `kyuz0/amd-strix-halo-toolboxes` image, also seen with `ghcr.io/ggml-org/llama.cpp:server-rocm`) the answer call intermittently returns the keyword JSON, an entity-extraction JSON or, with some models, garbage tokens, while `naive` queries and ingest keep working. It is not a prompt-cache artifact: it reproduces with `cache_prompt=false`, with a pinned slot, with JSON mode disabled and with the same request replayed by hand, and it happens with the 4B default as well as with `unsloth/Qwen3.6-35B-A3B-GGUF:Q4_K_XL`; the identical request replayed minutes later answers in prose. Harbor does not have a reliable mitigation from the LightRAG side, so:

- Run LightRAG's LLM on Ollama (`harbor up lightrag ollama`, optionally `tei` for embeddings), which is not affected and is what Harbor tests.
- If you need llama.cpp, use `naive` mode, or retry the query; a `harbor restart llamacpp` clears the state for a while.
- The graph itself is fine: `harbor up lightrag llamacpp tei` ingests a paragraph in about 30 s and the Knowledge Graph tab populates (`./services/lightrag/check-graph.sh --ingest-only`).
