...
- The prompt is too big for the model. With LightRAG's own defaults a knowledge base of ten small files already produces 25-60K-character prompts, on which `qwen3.5:4b` and `qwen3.5:9b` both scored 0 of 8 on a mixed corpus. Harbor ships `HARBOR_LIGHTRAG_OLLAMA_TOP_K=10`, `CHUNK_TOP_K=5` and `MAX_TOTAL_TOKENS=8000`; if you raised them, lower them again.
